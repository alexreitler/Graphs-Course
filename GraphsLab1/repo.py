...
class Graph:
    ...
    def list_isolated_vert(self):
        ...
        vertex_list = []
        for key in self.Din:
            if not self.Din[key] and not self.Dout[key]:
    ...
    def delete_vertex(self, vertex):
        ...
        if self.check_vertex(vertex):
            for i in self.Dout[vertex]:
                self.Dcost.pop((vertex, i))
                self.Din[i].remove(vertex)
            self.Dout.pop(vertex)

            for i in self.Din[vertex]:
                self.Dcost.pop((i, vertex))
                self.Dout[i].remove(vertex)
            self.Din.pop(vertex)
        else:
    ...
    def check_edge(self, vertex1, vertex2):
        ...
        if self.check_vertex(vertex1) and self.check_vertex(vertex2):
            if vertex2 in self.Dout[vertex1]:
                return True
            else:
                return False
        else:
...
class VertexIterator:
    ...
    def __init__(self, graph):
        self.list = list(graph.Dout.keys())
        self.value = 0
